...
class DocREModel(nn.Module):
    ...
    def get_hrt(self, sequence_output, cls_mask, attention, entity_pos, hts, offset, tag):
        ...
        hss = torch.cat(hss, dim=0)  # (num_ent_pairs_all_batches, Relation_Specific_num+1, emb_size)
        tss = torch.cat(tss, dim=0)  # (num_ent_pairs_all_batches, Relation_Specific_num+1, emb_size)
        rss = torch.cat(rss, dim=0)  # (num_ent_pairs_all_batches, Relation_Specific_num+1, emb_size)
    ...
    def forward(self,
        ...
        hs, rs, ts, Crs, bs_ht_num, doc_attn, batch_rel = self.get_hrt(sequence_output, attention_mask, attention,
                                                                              entity_pos, hts, offset, tag)
        doc_attn = doc_attn.mean(1)
        # doc_attn = self.get_hrt_doc_attn(sequence_output, attention, entity_pos, hts, offset)  # (num_ent_pairs_all_batches, max_doc_len)
