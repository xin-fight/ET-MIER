...
from model_SAIS_Evidence import DocREModel
        ...
        for epoch in tqdm(train_iterator, desc='Train epoch'):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if inputs["teacher_attns"] != None and attn_loss:
                    loss.append(outputs["loss"]["attn_loss"] * args.attn_lambda)  # teacher_attns
                if PEMSCLloss:
                    loss.append(outputs["loss"]["pemscl_loss"])
                if Relation_Seploss:
                    loss.append(outputs["loss"]["relation_seploss"])  # TCL loss
                if TASK_ET:
                    loss.append(outputs["loss"]["etoss"])  # ET loss
