...
def merge_results(pred: list, pred_pseudo: list, features: list, thresh: float = None):
    title2pred = get_title2pred(pred)  # dictionary with (key, value) = (title, {rel_triple: score})
    title2pred_pseudo = get_title2pred(pred_pseudo)

    title2gt = get_title2gt(features)  # dictionary with (key, value) = (title, [gold_triples])
...
def to_official(preds: list, features: list, evi_preds: list = [], scores: list = [], topks: list = []):
    ...
    # [{title:, h_idx, t_idx, r, evidence, score}]
    official_res = []
    res = []
